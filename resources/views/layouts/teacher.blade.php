...
<body>
  ...
  <script>
      ...
      if (role === 'admin') {
        ...
        if (meeraClasses.length === 0) {
          ...
          meeraClasses.forEach(cls => {
            ...
            div.innerHTML = `
              <div class="font-semibold"><a href="javascript:void(0)" class="text-primary hover-underline font-semibold" onclick="showStudentProfileModal('${cls.studentName}')">${cls.studentName}</a> - ${cls.instrument}</div>
    ...
    function renderDashboardCharts() {
      const role = db.getCurrentRole();
      if (role === 'teacher') {
        ChartManager.drawLineChart("teacherHoursChart", [14, 20, 18, 25, 24], ["Mar", "Apr", "May", "Jun", "Jul"]);
        ChartManager.drawBarChart("teacherStudentStats", [12, 6, 8, 4, 3], ["Vocal", "Sitar", "Violin", "Flute", "Tabla"]);
      }
    }
    ...
    function showStudentProfileModal(studentName) {
      const modal = document.getElementById("studentProfileModal");
      const body = document.getElementById("studentProfileModalBody");
      if (!modal || !body) return;

      const students = db.getStudents() || [];
      let std = students.find(s => s.name === studentName || s.name.includes(studentName));
      ...
      const embedUrl = getEmbedUrl(std.youtube);
      let youtubeHtml = "";
      if (embedUrl) {
        youtubeHtml = `
          <div class="mt-3" style="border-top: 1px solid var(--border-light); padding-top: 0.75rem;">
            <h4 class="font-bold" style="font-size: 0.85rem; margin-bottom: 0.4rem; color: var(--primary);">Practice Showcase Video</h4>
        ...
        youtubeHtml = `
          <div class="mt-3 p-3 text-center text-muted" style="border: 1px dashed var(--border-color); border-radius: var(--radius-md); font-size: 0.85rem; background: var(--bg-body);">
            🎥 No practice showcase video uploaded yet.
          </div>
        `;
      }
      ...
      body.innerHTML = `
        <div class="text-center mb-3">
          ...
          <h3 class="font-bold text-serif" style="font-size: 1.35rem; margin-bottom: 0.25rem;">${std.name}</h3>
          <span class="badge badge-success" style="font-size: 0.75rem;">Active Learner</span>
        </div>
        <div class="info-list-item" style="display:flex; justify-content:space-between; padding:0.65rem 0; border-bottom:1px solid var(--border-light); font-size:0.85rem;">
          ...
          <span class="font-bold">${std.id}</span>
        </div>
        <div class="info-list-item" style="display:flex; justify-content:space-between; padding:0.65rem 0; border-bottom:1px solid var(--border-light); font-size:0.85rem;">
          <span class="text-muted">Primary Instrument</span>
          <span class="font-semibold text-primary">${std.instrument}</span>
        </div>
        <div class="info-list-item" style="display:flex; justify-content:space-between; padding:0.65rem 0; border-bottom:1px solid var(--border-light); font-size:0.85rem;">
          <span class="text-muted">Available Credits</span>
          <span class="font-bold text-success">${std.credits} Credits</span>
        </div>
        <div class="info-list-item" style="display:flex; justify-content:space-between; padding:0.65rem 0; border-bottom:1px solid var(--border-light); font-size:0.85rem;">
          <span class="text-muted">Enrolled Since</span>
          <span class="font-medium">${std.enrolledDate || '2026-06-15'}</span>
        </div>
        ${youtubeHtml}
        <button class="btn btn-secondary w-100 mt-4" onclick="closeStudentProfileModal()">Close Profile</button>
      `;
  ...
  <div id="studentProfileModal" class="modal-backdrop">
    <div class="modal" style="max-width: 550px;">
      <div class="modal-header">
        <h3 class="font-semibold text-serif">Student Academic Profile</h3>
        <button class="modal-close" onclick="closeStudentProfileModal()">×</button>
      </div>
